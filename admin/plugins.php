							...
							<div class="col-lg-12">
								<div class="block">
									<table id="serverList" class="table table-striped table-sm">
										...
										<tbody>
											<?php foreach ($Plugins->pluginsList()['Response'] as $k => $v) { ?>
                                                <tr id="tr-id-<?php echo $v['id']; ?>" class="tr-class-<?php echo $v['id']; ?>" data-title="bootstrap table" data-object='{"key": "value"}' onclick="document.location.href = '/<?php echo $admDir; ?>/plugin?id=<?php echo $v['id']; ?>'" style="cursor:pointer;">
                                                    <td><a href="/<?php echo $admDir; ?>/plugin?id=<?php echo $v['id']; ?>"><?php echo $Secure->SecureTxt($v['Name']); ?></a></td>
													<td><img src="<?php echo $Games->gameByID($v['id'])['Icon']; ?>" style="padding-right:5px;" alt="<?php echo $Plugins->pluginsList($v['id'])['Name']; ?>" width="20px"><?php echo $Games->gameByID($v['gameID'])['Name']; ?></td>
													<td><?php if($v['Status'] == 0) { echo 'Private'; } else if($v['Status'] == 1) { echo 'Public'; } else { echo 'Undefined'; } ?></td>
													<td><?php echo $Admin->AdminDataID($Plugins->getPluginByID($v['id'])['createdBy'])['Name']." ".$Admin->AdminDataID($Plugins->getPluginByID($v['id'])['createdBy'])['Lastname']; ?></td>
